...
- **faixa decorativa de onda sonora removida do rodapé de cada seção hero**: com o ícone agora presente no cabeçalho de toda página, repetir o mesmo motivo na base de cada hero (Início, Pesquisa, Pessoas, Publicações, Notícias, Contato e Sistema de Design, em PT e EN) ficou redundante. Removida em todas as 11 ocorrências; o `Waveform.astro` continua em uso na própria página de Sistema de Design como exemplo documentado do motivo, e o padding da seção hero foi reduzido de volta ao normal já que não precisa mais reservar espaço para a faixa;
