...
- **logo do cabeçalho trocado para a assinatura oficial completa (ícone + wordmark)**: comparando `identidade-visual-fonUFAL.pdf` — cujas próprias aplicações de referência (papel timbrado, cartão de visita) usam sempre ícone + "fonUFAL" juntos, nunca só o wordmark — optamos pela assinatura primária em vez do wordmark isolado. Extraída com o mesmo método (geometria vetorial exata do `marca-fonUFAL.ai`, preservando o espaçamento e alinhamento de base originais entre ícone e letras) em `scrape/extract-logo-lockup-from-brand-file.py`, saída em `public/brand/fonufal-logo-lockup.svg` e componente `src/components/Logo.astro`; o rodapé continua com o wordmark isolado (`Wordmark.astro`), mais adequado ao espaço compacto ali.
